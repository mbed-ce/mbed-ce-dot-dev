...
*Note: This page was migrated from the [ARM Mbed 6 docs](https://os.mbed.com/docs/mbed-os/v6.16/contributing/license.html)*.

This page covers the different aspects of developing your own libraries for use in Arm Mbed devices, as well as items to keep in mind during development, such as licensing. It covers:
...
By creating a pull request on GitHub against Mbed OS, you agree to license your contributions under the same license as the original code. Mbed software is released under a permissive license for all new Mbed contributions. Mbed CE will not accept proprietary code or any code under a "copyleft" license. Accepting such code would make it difficult for anyone to use Mbed without releasing their own code as open source.
...
Note that some code in Mbed is licensed under an open source license that only permits use of the code with specific hardware or a specific brand of MCU. This is not recommended, but is allowed. If you contribute code with such a license, and it is outside the `mbed-os/targets/YOUR_COMPANY/YOUR_TARGET` folder (i.e. it's possible to build the code for an "illegal" target), then this must be CLEARLY documented, and the code should have #ifdef checks preventing its use on "illegal" platforms.

### License check in pull requests

We use a script to enforce the license in binaries and libraries when changes are submitted to Mbed CE. All source code files must contain an SPDX identifier and be licensed under an accepted open source license for the check to pass.

### Licensing your contributions

When submitting new files to Mbed, they must include a license header with an SPDX identifier. It's recommended to use the Apache-2.0 license unless the code already has a different license attached. See [above](#how-to-apply-apache-20-correctly) for the correct source file header to apply for Apache 2.0. 

The copyright statement should reflect the author of the code, whether an individual or a corporation.